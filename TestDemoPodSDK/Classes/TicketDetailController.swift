...
class TicketDetailController: UITableViewController {
            ...
            self.downloadTicketDetails(){
                ...
                if (dbObj?.open())! {
                    ...
                    while results?.next() == true {
                        ...
                        var fileName = attachmentArray[attachmentArray.count - 1]
                    
                        var date = "Reported on "+(results?.string(forColumn: "ticket_date"))!
                ...
                if (dbObj?.open())! {
                    ...
                    while results?.next() == true {
                        ...
                        var attachment : String = (results?.string(forColumn: "log_attachment"))!
                    
                        var attachmentArray = attachment.components(separatedBy: "/")
                    
                        var fileName = attachmentArray[attachmentArray.count - 1]
                    
                        var date = "Replied on "+(results?.string(forColumn: "log_date"))!
                    ...
                    for i in 0..<json["response"]["TicketLog"].count
                    {
                        ...
                        if (dbObj?.open())!
                        {
                            if String(describing: json["response"]["TicketLog"][i]["log_id"]) != "null"
                            {
                                ...
                                var logName = String(describing: json["response"]["TicketLog"][i]["first_name"])+" "+String(describing: json["response"]["TicketLog"][i]["last_name"])
                                
                                var attachments = String(describing: json["response"]["TicketLog"][i]["attachfiles"])
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if (segue.identifier == "addCommentSegue") {
            var viewController = segue.destination as! CommentController

            viewController.currentTicketID = self.currentTicketID
        }
        
        if (segue.identifier == "showAllSegue") {
            var viewController = segue.destination as! ViewAllAttachmentsController
