...
class ArticleController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
        ...
        var boldString = NSMutableAttributedString(string:self.articleData[indexPath.row]["Title"]!, attributes:attrs)
            ...
            let task = defaultSession.uploadTask(with: request, from: (data as! NSData) as Data, completionHandler:
                {(data,response,error) in
                    ...
                    for i in 0..<json["response"]["ARTICLE"].count
                    {
                        ...
                        var newArticleID: String! = String(describing: json["response"]["ARTICLE"][i]["article_id"])
                        ...
                        if (dbObj?.open())!
                        {
                            if let existingFaq = articleIDs.index(of:newArticleID)
                            {
                                ...
                                let updateSQL = "UPDATE TABLE_ARTICLE SET category_id = '\(String(describing: json["response"]["ARTICLE"][i]["category_id"]))', subcategory_id = '\(String(describing: json["response"]["ARTICLE"][i]["subcategory_id"]))', title = '\(String(describing: json["response"]["ARTICLE"][i]["article_title"]))', description = '\(String(describing: json["response"]["ARTICLE"][i]["article_description"]))', date = '\(String(describing: json["response"]["ARTICLE"][i]["modified_at"]))', rating = '\(rating)', comment_count = '\(String(describing: json["response"]["ARTICLE"][i]["comment"]))',  status = '1', totalviews = '\(String(describing: json["response"]["ARTICLE"][i]["totalviews"]))', article_like = '\(String(describing: json["response"]["ARTICLE"][i]["like"]))', article_unlike = '\(String(describing: json["response"]["ARTICLE"][i]["unlike"]))', article_like_flag = '0' WHERE article_id = '\(newArticleID)' "
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if (segue.identifier == "showArticleSegue") {
            var viewController = segue.destination as! ArticleDetailController
